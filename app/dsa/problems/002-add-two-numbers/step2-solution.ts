...
function addTwoNumbers(l1: ListNode | null, l2: ListNode | null): ListNode | null {
  // ✓ Step 1: Blank starter stub — the fixed clip point for the answer roll
  const starterStub = new ListNode(-1);
  let answerTail = starterStub;
  let overflowSatchel = 0; // carried value waiting for the next column
  ...
  while (l1 !== null || l2 !== null || overflowSatchel !== 0) {
    ...
    answerTail.next = new ListNode(columnTotal % 10); // stamp the ones digit onto the answer roll
    answerTail = answerTail.next;
    overflowSatchel = Math.floor(columnTotal / 10); // keep the overflow for the next column
