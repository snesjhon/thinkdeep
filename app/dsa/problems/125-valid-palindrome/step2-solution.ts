...
function isPalindrome(s: string): boolean {
  ...
  while (left < right) {
    // Step 2: Inspector Left skips empty pedestals (non-alphanumeric)
    while (left < right && !isAlphanumeric(s[left])) {
      left++;
    }

    // Step 2: Inspector Right skips empty pedestals (non-alphanumeric)
    while (left < right && !isAlphanumeric(s[right])) {
      right--;
    }

    // Comparison goes in step 3 — for now just advance past exhibits
    left++;
    right--;
  }
