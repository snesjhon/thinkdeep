...
function maxSlidingWindow(nums: number[], k: number): number[] {
  // ✓ Step 1: Back-clean + add (locked)
  const vistas: number[] = [];
  const notebook: number[] = [];

  for (let i = 0; i < nums.length; i++) {
    // Back-clean — erase hills shorter than the new arrival
    while (notebook.length > 0 && nums[notebook[notebook.length - 1]] <= nums[i]) {
      notebook.pop();
    }
    notebook.push(i);

    // Step 2: Front-clean — drop peak that scrolled past the telescope's left edge
    if (notebook[0] < i - k + 1) {
      notebook.shift(); // this hill is no longer in the window
    }

    // Report when window is full
    if (i >= k - 1) {
      vistas.push(nums[notebook[0]]); // front is always the tallest peak in view
    }
  }
